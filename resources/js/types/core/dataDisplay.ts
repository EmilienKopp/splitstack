...
type UseFormType<T> = typeof useForm<
    FormDataType<T extends object ? T : never>
>;
type InertiaForm<T> = ReturnType<UseFormType<T>>;
...
export type DataHeader<T, V = any> = {
    ...
    filterHandler?: (row: T, form: InertiaForm<T>) => boolean;
};
...
export interface IDataStrategy<T> {
    ...
    setFilters?(
        filters: {
            key: string;
            filterHandler:
                | ((row: T, form: InertiaForm<T>) => boolean)
                | undefined;
        }[],
    ): void;
}
...
export type TableConfig<T> = {
    ...
    filters?: {
        key: string;
        filterHandler: ((row: T, form: InertiaForm<T>) => boolean) | undefined;
    }[];
